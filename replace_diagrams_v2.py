...
print(f"Have {len(diagram_files)} new diagrams to insert")

# Replace images by updating the blob directly
for i, rel in enumerate(image_rels):
    if i < len(diagram_files):
        ...
        if os.path.exists(diagram_file):
            ...
            # Update the image part blob using internal attribute
...
print(f"Successfully replaced {min(len(image_rels), len(diagram_files))} diagrams")
